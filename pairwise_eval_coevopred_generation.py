...
if len(sys.argv) != 5:
    print >>sys.stderr, "USAGE: evaluation_replicate_generation.py <phase2_dir> <pred_spop> <evaluation_out_dir> <population_from_update>"
    sys.exit(1)

phase2_dir=sys.argv[1]
pred_spop=sys.argv[2]
phase3_dir=sys.argv[3]
seed_spop_update = sys.argv[4]
            ...
            for sgv_opp in sgvs:
                for ht_opp in hist_contings:
                    ...
                    for treatment in ["pred", "nopred"]:
                        ...
                        shutil.copytree(pairwise_skel_dir["nopred"], "/tmp/fishjord_tmp_phase3_pairwise_skel/")
                        ...
                        if treatment == "pred":
                            cmd.extend(["--predators", pred_spop])
                        print " ".join(cmd)
