...
if len(sys.argv) < 2:
    print >>sys.stderr, "solo_stats_phase1.py <rep_directory>..."
    sys.exit(1)

first_expected_update = 0
last_expected_update = 2000000
update_step = 1000

print "pred_history\treplicate\tupdate\tnum_prey\tmoves\tlooks\trotate\tprey_insts\tnum_pred\tpred_attacks"
for rep_dir in sys.argv[1:]:
    ...
    try:
        ...
        for i in range(len(targets)):
            ...
            print "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s" % (treatment, rep, update, num_prey, prey_inst[i]["move"], prey_inst[i]["look-ahead-intercept"], prey_inst[i]["rotate-x"], int(instructions), targets[i][2], pred_inst[i]["attack-prey"])
        ...
        if len(expected_updates) > 0:
            ...
            for update in sorted(expected_updates):
                print "{0}\t{1}\t{2}\t0\t0\t0\t0\t0\t0\t0".format(treatment, rep, update)
